...
class pokemon(db.Model):
    ...
    shiny_front = db.Column(db.String)
    hps = db.Column(db.Integer)
    atks = db.Column(db.Integer)
    defs = db.Column(db.Integer)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
                                #  notice          ^^^^^ --> lowercase?  yep.  User.id

    def __init__(self, name, ability, shiny_front, hps, atks, defs, user_id):
        self.name = name
        self.ability = ability
        self.shiny_front = shiny_front
        self.hps = hps
        self.atks = atks
        self.defs = defs
        self.user_id = user_id
